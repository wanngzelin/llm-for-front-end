...
export class OllamaController {
  ...
  @HttpCode(HttpStatus.OK)
  @Post('chatStream')
  async chatStream(@Body() chatStreamDto: ChatStreamDto, @Res() res: Response, @Req() req: Request) {
    if (!chatStreamDto.message) {
      throw new BadRequestException('会话不存在');
    }
    // 设置必要的响应头，确保流式输出不被缓存或压缩
    res.setHeader('Content-Type', 'text/plain; charset=utf-8');
    ...
    try {
      const stream = await this.ollamaService.streamChat(chatStreamDto)
